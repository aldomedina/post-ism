...
document.addEventListener("keydown", e => {
  ;(e.key === "Escape") & isProjectOpen ? closeProj() : null
})
...
function loadMore(data) {
  ...
  list_wrapper.insertAdjacentHTML(
    ...
    `<div class="list-item move-${mCounter}">
            <div class="post">POST-</div>
            <div class="listed-project" id="proj-${counter}">${data[counter].landTitulo}</div>
            <div class="ism">ISM</div>
        </div>`
  )
...
function showProject(data) {
  ...
  if (split_id[0] === "proj") {
    ...
    cover_img.insertAdjacentHTML("afterbegin", `<img src="${item.coverImg}">`)
    ...
    if (item.web) {
      cover_text.insertAdjacentHTML(
        "afterbegin",
        `<p><a class="web-anchor" href="${item.web}"> visitar website </a></p> <br><br></br>`
      )
    }
